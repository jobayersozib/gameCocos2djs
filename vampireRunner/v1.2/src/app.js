...
var HelloWorldLayer = cc.Layer.extend({
    ...
    animatehero:function(dt){
        ...
        if(cc.rectIntersectsRect(BoxZ,BoxH)){
            this.removeChild(this.zombie,true);
    ...
    fireVelocity:function(dt){
        ...
        for(var i=0;i<fireballs.length;i++){
            ...
            if((fireballs[i].getPosition().x<-50 && fireballs[i].getTag()==="fire") || (fireballs[i].getPosition().x>this.size.width && fireballs[i].getTag()==="friendly_fire")){
        ...
        for(var i=0;i<fireballs.length;i++){
            ...
            if(fireballs[i].getTag()==="friendly_fire"){
                 fireballs[i].setPosition(cc.p(fireballs[i].getPosition().x+7,fireballs[i].getPosition().y));  
            }
        ...
        var layer = new HelloWorldLayer();
         cc.eventManager.addListener({
            ...
            onKeyPressed:function(key,event){
                var friendlyFire= new cc.Sprite.create(res.fire);
                    friendlyFire.setTag("friendly_fire");
                    friendlyFire.setPosition(cc.p(layer.heroAltos.getPosition().x+5,layer.heroAltos.getPosition().y+10));
                    layer.addChild(friendlyFire,1);
            }
         },layer);
        this.addChild(layer);
    }
